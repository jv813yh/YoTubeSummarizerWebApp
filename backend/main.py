...
def get_transcript_text(video_id):
    ...
    for name, strategy in strategies:
        try:
            ...
            print(f"Strategy '{name}' failed with error: {error_type}: {e}") # Verbose Debug log
            
            # If the strategy failed because the video has no transcripts (but the method call worked), 
            # we should stop trying other strategies and tell the user.
            if error_type in ['TranscriptsDisabled', 'NoTranscriptFound', 'VideoUnavailable']:
...
async def send_email_async(email_to: str, summary: str, url: str):
    try:
        if not email_to:
            return
            
        html = f"""
        <h3>VendyGo Summary</h3>
        <p>Here is the AI summary you requested for the video: <a href="{url}">{url}</a></p>
        <hr>
        <div style="background-color: #f4f4f4; padding: 20px; border-radius: 10px;">
            {summary.replace(chr(10), '<br>')} 
        </div>
        <p><i>Generated by VendyGo Tools</i></p>
        """
...
async def summarize_video(request: SummarizeRequest, background_tasks: BackgroundTasks):
    ...
    if request.email:
        background_tasks.add_task(send_email_async, request.email, final_summary, request.url)
